...
def my_single_worker_inference(infer_model, ckpt, inference_input_file, inference_output_file, hparams):
   output_infer = inference_output_file
   f = open(inference_input_file,encoding='utf-8')
   line = f.readline()
   with tf.Session(graph=infer_model.graph, config=utils.get_config_proto()) as sess:
       ...
       while line:
           ...
           if l == 2:
               ...
               uc_txt = fs[0]
           #print("start="+uc_txt+"=end")
           iterator_feed_dict = {infer_model.src_placeholder: [uc_txt],infer_model.batch_size_placeholder: 1,}
           sess.run(infer_model.iterator.initializer, feed_dict=iterator_feed_dict)
           nmt_outputs, attention_summary = loaded_infer_model.decode(sess)
